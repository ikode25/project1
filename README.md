...
- **Page navigation on the longer tables** — Customers, Appointments (table view), Users, Reviews, and SMS History all paginate at 15 rows/page once a list grows past that, instead of dumping everything into one long scroll; the same `renderTable(..., pageKey)` helper can be dropped onto any other list on request
...
- **Branding & Theme, now organized into tabs** — Branding (now with the business logo also shown next to the name in the admin sidebar itself, not just the public site) and Theme & Colors (with a **site font picker** — six curated web-safe font stacks with a live preview, no external font download), Social & Contact, Booking, Payments & Loyalty (now also holds **Accepted Payment Methods** — a tap-to-toggle chip picker controlling exactly which of Cash/MTN MoMo/Vodafone Cash/Telecel Cash/AirtelTigo Money/Bank Transfer customers are offered at checkout — plus the MoMo number/name and bank details shown to customers who pick one), SMS Gateway, and QR Code
