...
- Multi-branch selector — or a simple single-shop layout if the Owner turns off "multiple branches" in Settings (not every barbershop has more than one location)
...
- Contact section: click-to-call, WhatsApp deep link, embedded Google Map, a full **weekly working-hours table** (per-day open/close times, 24-hour days, or closed — set once in Settings → Booking, shown to every customer — or per-branch hours if the business has multiple branches), **social media icon row** (only shows platforms the Owner has filled in)
- A live **Open Now / Closed Now** badge next to the business name in the navbar — computed server-side from the weekly working hours in Africa/Accra time, so it's accurate regardless of the visitor's own timezone. The same badge appears in the admin topbar for every role, so staff always know at a glance whether the shop is currently open
...
- **Branding & Theme, now organized into tabs** — Branding (now with the business logo also shown next to the name in the admin sidebar itself, not just the public site) and Theme & Colors (with a **site font picker** — six curated web-safe font stacks with a live preview, no external font download), Social & Contact, Booking (now also asks **"Does your business have multiple branches?"** — turning this off renames Branches to a simple Shop Info page and hides the branch picker everywhere; Working Hours here double as what's shown to customers on the public site), Payments & Loyalty (now also holds **Accepted Payment Methods** — a tap-to-toggle chip picker controlling exactly which of Cash/MTN MoMo/Vodafone Cash/Telecel Cash/AirtelTigo Money/Bank Transfer customers are offered at checkout — plus the MoMo number/name and bank details shown to customers who pick one), SMS Gateway, and QR Code. **Saving settings now does one bulk read/write instead of ~30 separate ones** — the single biggest cause of "Save All Settings" feeling slow
...
- **Uploaded photos/logos use Drive's `thumbnail` endpoint, not `uc?export=view`.** The latter can serve an interstitial "can't scan for viruses" HTML page instead of the actual image bytes when hotlinked from an `<img>` tag — intermittently, and more often for larger files — which is why an uploaded logo could fail to show up on the site even though the upload itself succeeded. `uploadImageToDrive_()` now returns a `drive.google.com/thumbnail?id=...&sz=w1000` URL instead, which reliably serves real image bytes for anyone-with-link-view files. This affects every photo picker in the app (logo, staff, services, products, hero slides, gallery) — already-uploaded images keep their old URL and are unaffected either way (Drive still serves them, just less reliably), but every image uploaded from now on gets the more reliable link.
...
- **One weekly-hours source of truth drives everything time-related.** `getAvailableSlots()` (public booking) and `getSlotsForAdmin()` (Manage Slots) both derive that date's start/end minute window — and whether the day is bookable at all — from `Settings.WeeklyHours` for that weekday, instead of a flat all-week start/end hour. A day the shop marks closed is gated out before staff schedules are even considered, both in what's offered and in `createAppointment()`'s own server-side re-validation (`isSlotAvailable_`), so the public wizard, Manage Slots, and the actual booking guard can never disagree about whether a given day is open.
