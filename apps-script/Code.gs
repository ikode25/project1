...
function batchUpdateRemarks(token,arr){
  ...
  try{
    ...
    for(var i=1;i<data.length;i++){if(data[i][0])idMap[data[i][0].toString()]=i+1;}
    ...
    arr.forEach(function(item){
      var row=idMap[item.StudentID.toString()];
      if(row){
        ...
        // Write snapshot to RemarksArchive
        var studentName = data[row-1][1];
        var className = data[row-1][3];
        var key = item.StudentID.toString() + '_' + period.year + '_' + period.term;
        var archiveRow = archiveMap[key];
        var rowValues = [item.StudentID.toString(), studentName, className, period.year, period.term, attVal, outVal, interestVal, conductVal, attitudeVal, ctRem, htRem, promo];
        if (archiveRow) {
  ...
  try{
    ...
    arr.forEach(function(item){
      ...
      if(row){
        ...
        // Write snapshot to FeesArchive
        var studentName = data[row-1][1];
        var className = data[row-1][3];
        var key = item.StudentID.toString() + '_' + period.year + '_' + period.term;
        var archiveRow = archiveMap[key];
        var rowValues = [item.StudentID.toString(), studentName, className, period.year, period.term, arrVal, nxtVal, feeDataString];
        if (archiveRow) {
...
function executeAutomaticPromotions(token, targetYear, targetTerm) {
  ...
  try {
    ...
    // Helper for automatic class progression
    function getNextClass(className) {
      ...
      if (jhsMatch) {
        var num = parseInt(jhsMatch[1]);
        if (num === 3) return 'Graduated / Alumni';
        return 'JHS ' + (num + 1);
      }
